...
void dilatation_paralela(img_data* image, int n_task){
	...
	//Reservando memoria
	image->img_c= (int **)malloc (image->width*sizeof(int *));
	for ( i= 0; i < image->width; i++)  // columnas
		image->img_c[i] = (int *) malloc (image->width*sizeof(int));

	//copia de la imagen original
	for( i = 0; i < image->width; i++ )
  	{
      for( j = 0; j < image->width; j++ ){
        image->img_c[i][j]= image->img[i][j];
      }
  	}

  	//dilation
  	__m128i up,down,left,right,center,sum, pixel;
  	#pragma omp parallel num_threads(n_task) private(j,up,down,left,right,center,sum,pixel)
    {
	    ...
	    for(i = 1; i< image->width-1; i++){
	        ...
	        for(j = 1; j < image->width-4; j= j+4){
	            ...
	            _mm_storeu_si128((__m128i*)&image->img_c[i][j],center);
